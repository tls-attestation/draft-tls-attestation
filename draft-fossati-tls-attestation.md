...
such as hardware-enforced time limitations, or use of platform-level APIs in the case of cloud infrastructure. While full peer authentication is not available in this mode, the TIK provides continuity of identity between TLS sessions with the same peer.
...
We note that this protocol as-is only guarantees that the Identity Key is known by the TEE. A number of additional guarantees must be provided by the platform and/or the TLS stack,
