...
In addition, the design supports two ways of identifying the peer: a mode that combines normal X.509 certificate authentication with platform attestation, as well as an attestation-only mode.

This document does not specify any particular attestation technology.
...
Attestation alone is vulnerable to identity spoofing attacks, in particular when zero-day attacks exist for a class of hardware. [Need a reference]. Therefore it needs to be combined with traditional authentication, which in the case of TLS takes the form of X.509 certificates.

We RECOMMEND that regular applications only use the combined mode, which provide the full security guarantees on an authenticated TLS handshake (for the peer/peers being authenticated) as
well as guarantees on platform integrity.

The attestation-only mode is provided for specialized use cases, including initial provisioning of the TLS stack. In these cases additional security controls SHOULD be provided,
...
We note that this protocol as-is only guarantees that the Identity Key is seen by the TEE. A number of additional guarantees must be provided by the platform and/or the TLS stack,
...
* The TLS protocol, whether implemented by the TEE or a layer above it, is implemented correctly and (for example) does not leak any session key material.

Assurance of these properties, such as through providing source code, reproducible builds, formal verification, correct definition of the scope of attestation measurements etc. is out of scope of this document.
